...
12. [부록 A: Long Polling 심화](#부록-a-long-polling-심화---http-vs-grpc-vs-websocket)
...
#### 3.4.1 Task Token과 Worker 작업 격리

> **"같은 Task를 두 Worker가 동시에 처리할 수 있나요?"**

절대 불가능합니다. Temporal은 **Task Token**을 통해 작업 소유권을 완벽하게 보장합니다.
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│   1. Worker A가 Task를 가져감                                               │
│      └─ Temporal이 Task Token 발급: "ABC123"                                │
│                                                                              │
│   2. Worker A가 결과 반환 시                                                 │
│      └─ "Token ABC123으로 결과입니다" (소유권 증명)                          │
│                                                                              │
│   3. Worker B가 같은 Task 결과를 반환하려 하면?                              │
│      └─ ❌ 실패! (Token이 없거나 다름)                                       │
│                                                                              │
│   Task Token = 작업에 대한 "열쇠"                                           │
│   열쇠 없이는 결과를 반환할 수 없음                                          │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
```
[Worker A와 Worker B가 같은 Task Queue를 감시하는 경우]
...
│
│  결과:
│  - Worker A: Task 1 처리 (Token AAA 소유)
│  - Worker B: Task 2 처리 (Token BBB 소유)
│  - 작업이 "뒤섞이는" 일 없음!
```

#### 3.4.2 Worker 장애 시 처리 - Timeout과 새 Token

> **"그런데 Worker가 죽으면 다른 Worker가 이어받잖아요? 이게 상충 아닌가요?"**

상충이 아닙니다. **시간에 따른 두 가지 다른 상황**입니다.
...
[정상 케이스: Task Token이 보호하는 상황]
─────────────────────────────────────────
시간 →
│
│  Worker A가 Task 획득          Worker A가 결과 반환
│         ↓                              ↓
│    ┌─────────────────────────────────────┐
│    │     Task Token ABC123 유효          │
│    │     (다른 Worker 접근 불가)          │
│    └─────────────────────────────────────┘
│              ↑
│         이 구간에서 Worker B가 같은 Task를
│         가로챌 수 없음 (Token 소유권)


[장애 케이스: Timeout 후 재할당]
─────────────────────────────────────────
시간 →
│
│  Worker A가 Task 획득    Worker A 장애!     Timeout 만료     Worker B가 새 Task 획득
│         ↓                    ↓                  ↓                    ↓
│    ┌──────────────────────────────┐        ┌──────────────────────────────┐
│    │   Task Token ABC123 유효     │   →    │   Task Token XYZ789 (새것!)   │
│    │   Worker A 소유              │        │   Worker B 소유               │
│    └──────────────────────────────┘        └──────────────────────────────┘
│                                    ↑
│                           여기서 핵심 전환 발생:
│                           1. ABC123 토큰 만료/무효화
│                           2. Temporal이 새 Task 생성
│                           3. 새 토큰 XYZ789 발급
│                           4. Worker B가 새 Task 획득
```
...
| 상황 | Task Token | 동작 |
|------|-----------|------|
| **정상 처리 중** | 유효 (Active) | 다른 Worker가 **같은 Token의 Task**를 가져갈 수 없음 |
| **Timeout 후** | 만료 (Expired) | Temporal이 **새 Token으로 새 Task** 생성 → 다른 Worker 할당 가능 |
...
1. Worker A가 Task 획득
   - Task Token: ABC123
   - Start-to-Close Timeout: 60초

2. Worker A가 30초 후 장애 발생 (네트워크 단절, 프로세스 크래시 등)
   - ABC123 토큰으로 결과를 반환할 수 없음

3. 60초 Timeout 만료
   - Temporal Server: "ABC123 토큰 결과 없음 → Task 실패 처리"
   - ABC123 토큰은 이제 무효

4. Temporal이 재시도 결정 (RetryPolicy에 따라)
   - 새 Task 생성
   - 새 Token: XYZ789 발급

5. Worker B가 새 Task 획득
   - XYZ789 토큰 소유
   - 이것은 "가로채기"가 아니라 "새 작업 할당"
```
...
```java
// 이런 상황이 불가능하다는 것:
Worker A: processPayment() 실행 중... (Token: ABC123)
Worker B: "내가 ABC123 결과를 대신 반환할게!" ← 불가능!

// 이런 상황은 가능:
Worker A: processPayment() 실행 중... (Token: ABC123)
[60초 후 Timeout]
Temporal: "ABC123 실패, 새 Task 생성 (Token: XYZ789)"
Worker B: processPayment() 새로 실행 (Token: XYZ789) ← 가능!
```

**정리:**
- **Task Token**은 "진행 중인 작업의 소유권"을 보호
- **장애 시 재할당**은 "새 작업 생성"으로 처리
- 같은 작업 "탈취"가 아니라 동일 Activity의 "재시도"(새 인스턴스)
...
## 부록 A: Long Polling 심화 - HTTP vs gRPC vs WebSocket

> Temporal Worker가 사용하는 Long Polling 기법에 대한 심화 학습

### A.1 Long Polling이란?

> **"서버야, 할 일 생기면 알려줘. 그때까지 기다릴게."**

Long Polling은 클라이언트가 서버에 요청을 보내고, 서버가 **즉시 응답하지 않고 데이터가 준비될 때까지 연결을 유지**하는 기법입니다.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    Short Polling vs Long Polling 비교                        │
└─────────────────────────────────────────────────────────────────────────────┘
...
[Long Polling - Temporal 방식]
─────────────────────────────
Worker                                    Temporal Server
   │                                            │
   │── "Task 있으면 알려줘 (60초 기다릴게)" ───▶│
   │            (연결 유지... 대기...)           │
   │            (30초 경과...)                  │
   │              ⭐ 새 Task 도착!               │
   │◀─────────────────── "Task 있어! 여기!" ────│
...
### A.2 HTTP vs gRPC Long Polling

**Long Polling은 HTTP에서 먼저 시작된 기술입니다.** gRPC만 가능한 것이 아닙니다.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                   HTTP/1.1 vs HTTP/2 (gRPC) 비교                             │
└─────────────────────────────────────────────────────────────────────────────┘

[HTTP/1.1 Long Polling]
───────────────────────
Worker ════ TCP 연결 1 ════▶ Server (Poller 1)
       ════ TCP 연결 2 ════▶ Server (Poller 2)
       ════ TCP 연결 3 ════▶ Server (Poller 3)

→ 동시 요청마다 별도 TCP 연결 필요
→ 헤더 매번 전송 (중복)


[gRPC (HTTP/2) Long Polling]
────────────────────────────
Worker ════════════ 단일 TCP 연결 ════════════▶ Server
            │                              │
            ├─ Stream 1 (Poller 1) ────────┤
            ├─ Stream 2 (Poller 2) ────────┤
            └─ Stream 3 (Poller 3) ────────┘
...
**Temporal이 gRPC를 선택한 이유:**

| 기능 | HTTP/1.1 | gRPC (HTTP/2) | Temporal에 중요한 이유 |
|------|----------|---------------|----------------------|
| **멀티플렉싱** | ❌ | ✅ | Worker가 여러 Poller 동시 운영 |
| **헤더 압축** | ❌ | ✅ | 반복 요청 시 대역폭 절약 |
| **바이너리 프로토콜** | ❌ (텍스트) | ✅ (Protobuf) | 직렬화/역직렬화 성능 |
| **타입 안전성** | ❌ | ✅ (.proto) | API 계약 명확 |
| **코드 생성** | ❌ | ✅ | 다양한 언어 SDK 자동 생성 |

### A.3 Long Polling vs WebSocket (FE ↔ BE 통신)

> **참고**: Temporal은 백엔드 Worker 통신에 Long Polling을 사용합니다.
> 프론트엔드-백엔드 통신에서는 상황에 따라 선택이 달라집니다.
...
**성능 비교 (1000개 메시지 전송 시):**

| 항목 | WebSocket | Long Polling |
|------|-----------|--------------|
| 연결 수립 | 1회 | 1000회 |
| 메시지당 오버헤드 | 2-6 bytes | ~500 bytes |
| 총 오버헤드 | ~6KB | ~500KB |
| 연속 메시지 지연 | ~1-5ms | ~50-100ms |

**언제 무엇을 선택?**

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                   Long Polling을 고려할 수 있는 상황                         │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  1. 메시지 빈도가 매우 낮음 (분당 1-2회 이하)                                │
│     └─ 예: 배치 작업 완료 알림, 일일 리포트 알림                             │
│                                                                              │
│  2. 레거시 인프라 제약                                                       │
│     └─ 프록시가 WebSocket 미지원                                             │
│     └─ 방화벽이 Upgrade 헤더 차단                                            │
│                                                                              │
│  3. Serverless 환경 (AWS Lambda 등)                                          │
│     └─ 연결 상태 유지 어려움                                                 │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────────┐
│                      WebSocket이 명확히 우위인 상황                          │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  1. 실시간 채팅 - 메시지 빈도 높음, 즉시 전달 필요                           │
│  2. 실시간 협업 (Google Docs, Figma) - 양방향 통신, 낮은 지연 필수           │
│  3. 게임 - 초당 수십 회 이상 통신                                            │
│  4. 실시간 대시보드/모니터링 - 지속적인 데이터 스트림                        │
│  5. 주식/암호화폐 시세 - 초당 수백 건 업데이트                               │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| 질문 | WebSocket | Long Polling |
|------|-----------|--------------|
| 메시지 빈도 초당 1회 이상? | ✅ | ❌ |
| 양방향 통신 필요? | ✅ | ❌ |
| 서버에서 즉시 Push 필요? | ✅ | ❌ |
| 인프라가 WebSocket 지원? | ✅ | 미지원 시 고려 |
| Serverless 환경? | ❌ | ✅ |

**권장 사항:**
- **기본 선택**: WebSocket (2024년 현재 대부분 환경에서 지원)
- **안전한 접근**: SockJS나 Socket.IO 사용 → 자동 Fallback 지원
- **Temporal 방식**: 백엔드 Worker 통신에는 gRPC Long Polling이 최적
