...
## 10. Temporal 미리보기: Saga가 이렇게 쉬워진다

> **Phase 3 예고**: 지금까지 구현한 복잡한 Saga 코드가 Temporal에서 어떻게 단순해지는지 미리 살펴봅니다.

### 10.1 현재 구현의 복잡성 (우리가 만든 것)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                     REST Saga Orchestrator (Phase 2-A)                      │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   직접 구현해야 하는 것들:                                                  │
│                                                                             │
│   1. Saga 상태 저장소 (DB 테이블 + 엔티티)                                  │
│   2. 상태 전이 로직 (PENDING → INVENTORY_RESERVED → ...)                    │
│   3. 보상 트랜잭션 호출 순서 관리                                           │
│   4. 실패 시 롤백 로직 (어디까지 성공했는지 추적)                            │
│   5. 중간 장애 복구 (스케줄러로 미완료 Saga 탐색)                            │
│   6. 타임아웃 처리 (각 단계별 타임아웃)                                      │
│   7. 재시도 로직 (Resilience4j 설정)                                        │
│   8. 멱등성 보장 (Idempotency-Key 관리)                                     │
│                                                                             │
│   예상 코드량: ~500줄 이상                                                  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
### 10.2 Temporal로 구현하면 (Phase 3에서 경험)

```java
/**
 * Temporal Workflow - 보상 트랜잭션이 자동으로 관리됨
 */
@WorkflowInterface
...
public class OrderSagaWorkflowImpl implements OrderSagaWorkflow {
    ...
    // Activity stubs - Temporal이 재시도, 타임아웃 자동 처리
    ...
    public OrderResult processOrder(OrderRequest request) {
        // Saga.run()으로 보상 트랜잭션 자동 관리!
        Saga saga = new Saga(new Saga.Options.Builder().build());

        try {
            // 1. 재고 차감 + 보상 등록
            saga.addCompensation(() -> inventory.releaseStock(request));
            inventory.reserveStock(request);

            // 2. 결제 처리 + 보상 등록
            saga.addCompensation(() -> payment.refund(request));
            payment.charge(request);

            return OrderResult.success();

        } catch (Exception e) {
            // 실패 시 등록된 보상이 역순으로 자동 실행!
            saga.compensate();
            throw e;
        }
    }
...
├────────────────────────────────┼────────────────────────────────────────────┤
│                                │                                            │
│  ❌ saga_state 테이블 필요      │  ✅ Temporal이 상태 자동 저장              │
│                                │                                            │
│  ❌ SagaState 엔티티/Mapper    │  ✅ 불필요 - Workflow가 상태               │
│                                │                                            │
│  ❌ 상태 전이 enum 관리        │  ✅ 코드 흐름 = 상태                        │
│                                │                                            │
│  ❌ 보상 순서 직접 관리        │  ✅ saga.addCompensation()으로 자동         │
│                                │                                            │
│  ❌ 복구 스케줄러 필요         │  ✅ Temporal Worker가 자동 복구             │
│                                │                                            │
│  ❌ Resilience4j 재시도 설정   │  ✅ RetryOptions 내장                       │
│                                │                                            │
│  ❌ 멱등성 키 테이블 필요      │  ✅ Workflow ID로 자동 중복 방지            │
│                                │                                            │
│  ❌ 타임아웃 수동 관리         │  ✅ ActivityOptions.timeout 내장            │
│                                │                                            │
├────────────────────────────────┼────────────────────────────────────────────┤
│  코드량: 500줄+                │  코드량: ~100줄                            │
│  테이블: 2개+                  │  테이블: 0개 (Temporal이 관리)             │
└────────────────────────────────┴────────────────────────────────────────────┘
```

### 10.4 Temporal Saga의 핵심 개념
...
│                                                                             │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │ saga.addCompensation(() -> inventory.release());                    │   │
│   │ inventory.reserve();  // Step 1 실행                               │   │
│   │                       ↓                                             │   │
│   │ saga.addCompensation(() -> payment.refund());                       │   │
│   │ payment.charge();     // Step 2 실행                               │   │
│   │                       ↓                                             │   │
│   │ saga.addCompensation(() -> shipping.cancel());                      │   │
│   │ shipping.schedule();  // Step 3 실행 ← 여기서 실패!                 │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│   catch (Exception e) {                                                     │
...
│                           ↓                                                 │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │ 1. payment.refund()   ← 2번째로 등록된 보상                        │   │
│   │ 2. inventory.release() ← 1번째로 등록된 보상                       │   │
│   │ (shipping.cancel은 실행 안 함 - schedule이 실패했으므로)            │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 10.5 왜 Phase 2-A를 먼저 하는가?
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   Phase 2-A에서 직접 구현하면서 겪는 어려움:                                 │
│                                                                             │
│   1. "보상 순서 실수하면 데이터 불일치 발생"                                 │
│      → Temporal: addCompensation 순서대로 자동 관리                        │
│                                                                             │
│   2. "서버 다운되면 진행 중인 Saga는?"                                       │
│      → Temporal: Worker 재시작 시 자동 복구                                 │
│                                                                             │
│   3. "재시도 로직 구현 복잡"                                                 │
│      → Temporal: RetryOptions 몇 줄로 해결                                  │
│                                                                             │
│   4. "멱등성 키 테이블 관리 번거로움"                                        │
│      → Temporal: Workflow ID가 자동 멱등성 보장                             │
│                                                                             │
│   5. "디버깅할 때 Saga 상태 추적 어려움"                                     │
│      → Temporal: Web UI에서 전체 히스토리 시각화                            │
│                                                                             │
│   ═══════════════════════════════════════════════════════════════════════   │
│                                                                             │
│   💡 어려움을 직접 경험해야 Temporal의 가치를 체감할 수 있습니다!            │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 10.6 Phase 3에서 확인할 것들

- [ ] Saga.java 유틸리티 클래스 사용법
- [ ] ParallelCompensation 옵션 (보상 병렬 실행)
- [ ] ContinueWithError 옵션 (일부 보상 실패해도 계속)
- [ ] Temporal Web UI에서 Saga 진행 상황 모니터링
- [ ] Phase 2-A 코드와 1:1 비교
...
- [Temporal Java SDK - Saga](https://www.javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/workflow/Saga.html)
