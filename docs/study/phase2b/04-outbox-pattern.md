...
│     - 1초 지연이 허용되는 경우                                               │
│                                                                              │
│  3. 인프라 복잡도를 낮추고 싶은 경우                                         │
│     - Debezium 운영 부담 없음                                                │
│     - DB 외 추가 컴포넌트 불필요                                             │
│                                                                              │
│  4. 단일 인스턴스 환경                                                       │
│     - FOR UPDATE SKIP LOCKED로 충분                                          │
│                                                                              │
│  ─────────────────────────────────────────────────────────────────────────  │
│                                                                              │
│  ✅ CDC (Debezium) 방식이 적합한 경우:                                       │
│  ────────────────────────────────────                                        │
│                                                                              │
│  1. 실시간성이 중요한 경우                                                   │
│     - 100ms 이내 이벤트 전파 필요                                            │
...
│  2. 대량 이벤트 처리                                                         │
│     - 초당 수천 건 이상                                                      │
│     - Polling으로 인한 DB 부하 우려                                          │
│                                                                              │
│  3. 순서 보장이 중요한 경우                                                  │
│     - binlog 순서 = 커밋 순서                                                │
│     - 추가 정렬 로직 불필요                                                  │
│                                                                              │
│  4. 이미 Kafka/CDC 인프라가 있는 경우                                        │
│     - 기존 Debezium 활용                                                     │
│     - 표준화된 이벤트 파이프라인                                             │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
...
|------|---------|----------------|
| **지연** | 폴링 주기 (1초~) | ~100ms |
| **DB 부하** | 있음 (주기적 쿼리) | 없음 (로그 읽기) |
| **순서 보장** | 추가 처리 필요 | 자동 (binlog) |
| **구현 복잡도** | 낮음 | 중간 |
| **인프라** | 없음 | Debezium Server |
| **확장성** | SKIP LOCKED로 제한적 | 자연스러운 확장 |
| **학습 곡선** | 낮음 | 높음 |
...
1️⃣ Polling 먼저 구현
   → Outbox 패턴 핵심 이해
   → FOR UPDATE SKIP LOCKED 학습
   → 한계 직접 체험

2️⃣ CDC로 전환 (선택)
   → 실시간 처리 비교
   → Debezium 설정 경험
   → 04-1-cdc-debezium.md 참조
```
