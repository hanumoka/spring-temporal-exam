...
| D026 | 결제 시점 전략 | 2단계 결제 (Authorization → Capture) |
...
**결정**: 2단계 결제 패턴 (Authorization → Capture) 권장
...
│  T2: 재고 예약     ✅                                                    │
│  T3-1: 결제 생성   ✅                                                    │
│  T3-2: 결제 승인   ✅  ◄─── 💰 실제 돈 인출 (PG사 API 호출)              │
│  T4: 주문 확정     ❌  ◄─── 여기서 실패하면?                              │
│                                                                          │
│  문제:                                                                   │
│  ├── 고객 돈은 이미 빠졌는데 주문은 PENDING 상태                          │
│  ├── 환불 처리 필요 (C3: refundPayment)                                  │
│  └── 환불에 시간 소요, 카드 취소 수수료 발생 가능                         │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| 단계 | 메서드 | 실제 돈 인출 | 비고 |
|------|--------|-------------|------|
| T3-1 | `createPayment()` | ❌ | DB 저장만 |
| **T3-2** | **`approvePayment()`** | **✅** | **PG사 API 호출 = 실제 결제** |
| T6 | `confirmPayment()` | ❌ | 내부 상태 변경만 |

### 권장 패턴: 2단계 결제 (Authorization + Capture)
...
### 2단계 결제 적용 시 Saga Flow

```
기존 Flow (1단계 결제):
  T3-2: approvePayment() ← 💰 돈 빠짐
  T4~T6: ...
  실패 시: refundPayment() ← 💸 환불 (시간 소요)

개선 Flow (2단계 결제):
  T3-2: authorizePayment() ← 💳 카드 홀딩 (돈 안 빠짐)
  T4~T5: ...
  T6: capturePayment() ← 💰 실제 청구 (Saga 완료 확정 후)
  중간 실패 시: voidPayment() ← ❌ 홀딩 취소 (즉시, 수수료 없음)
```
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  [학습 Phase 2-A] 현재 구현 유지                                         │
│  ├── 1단계 결제로 "문제 체험" 목적 달성                                   │
│  ├── 환불 보상 트랜잭션의 복잡성 체험                                    │
│  └── Fake PG에서 지연/실패 시뮬레이션                                    │
│                                                                          │
│  [Fake PG 구현 시 고려사항]                                              │
│  ├── 현재: approvePayment() 단일 메서드                                  │
│  ├── 확장: authorize() / capture() / void() 분리 가능                    │
│  └── 두 패턴 모두 테스트 가능하도록 설계                                 │
│                                                                          │
│  [실무 적용 시] 2단계 결제 권장                                          │
│  ├── PG사 SDK가 대부분 2단계 지원 (토스, 나이스, 카카오페이 등)           │
│  └── Saga 중간 실패 시 깔끔한 처리 가능                                  │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

### Fake PG 인터페이스 설계 (Day 1 구현 시)
...
│                                                                          │
│  2단계 결제로 해결:                                                       │
│  ├── 모든 Saga 단계 완료 후에만 실제 청구                                 │
│  ├── 중간 실패 시 홀딩 취소 (void) = 즉시, 무료                          │
│  └── 고객 경험 개선                                                      │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
- [D015. 외부 서비스 시뮬레이션 전략](#d015-외부-서비스-시뮬레이션-전략) - Fake PG 구현
- [01-saga-pattern.md](../study/phase2a/01-saga-pattern.md) - Saga 패턴 학습
